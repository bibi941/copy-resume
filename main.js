...
function getCurrtentWindow() {
  ...
  for (let i = 1; i < target_anchor.length; i++) {
    if (Math.abs(target_anchor[i].offsetTop - window.scrollY) < Math.abs(target_anchor[minIndex].offsetTop - window.scrollY)) //先假设第0个最小,开始遍历的offsetTOP，再比较 {
      minIndex = i
  }
...
function topBar_Anchor() {
  ...
  for (let i = 0; i < aTags.length; i++) {
    aTags[i].onclick = function (x) {
      ...
      //tween.js库
      function animate(time) { //自动计算多少毫秒动一次
